...
import org.lastbamboo.platform.sdp.GenericObject;
import org.lastbamboo.platform.sdp.GenericObjectList;
import org.lastbamboo.platform.sdp.InternalErrorHandler;
import org.lastbamboo.platform.sdp.Match;
		...
		for (int i = 0; i < fields.length; i++) {
			...
			try {
				...
				if (fieldType.isPrimitive()) {
					...
					GenericObjectList.class.isAssignableFrom(
						fieldType)) {
					if (f.get(this) != null) {
						sprint(
